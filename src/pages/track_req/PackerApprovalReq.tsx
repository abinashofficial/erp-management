...
import { PackerApproval } from "../raise-req/PackerApproval";
import PackagingList from '@mui/icons-material/ListAltOutlined';
import { PackerReportingRequest } from "../raise-req/PackerReporting";
...
export function PackerApprovalRequest() {
  const [searchTerm, setSearchTerm] = useState('');
    const { packagingApprovalRequestData,setPackerCheckList } = useContext(locateContext);
  ...
  // const data = packagingApprovalRequestData // Provide your data here
  ...
  const filteredRows = packagingApprovalRequestData.filter((row: any) => {
  ...
  const [reportReq, setReportReq] = useState(false);
...
// );
  ...
  const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement>, field: string) => {
    ...
    const checkList = (type: String, reqId: string) => {
      setPackerCheckList(reqId)
      if (type === "Ground Components"){
        navigate("/Packaging_Management/checklist/groundcomponent/edit")
      }else if(type === "Flight Components"){
        navigate("/Packaging_Management/checklist/flightcomponent/edit")
      }else if(type === "Consumable Components"){
        navigate("/Packaging_Management/checklist/consumablecomponent/edit")
      }
    };
          ...
          <Table>
            ...
            <TableRow  >
              ...
              <TableRow >
                ...
                <StyledTableCell align="center">Packer Name</StyledTableCell>
...
{item.status === "Inventory Lead Approved" ?                             (
...
{item.status === "Approved for Packaging" || item.status === "Repack" ?                             (
        <InventoryOutlinedIcon
          onClick={() => checkList(item.package_type, item.name)}
          style={{ cursor: 'pointer' }}
        />
      ): (
        <InventoryOutlinedIcon style={{ color: 'grey' }} />
                      ...
                      {/* <VisibilityTwoToneIcon onClick={()=>userAction(item, "visible")}/> */}

      {item.status === "Approved for Packaging" || item.status === "Repack" ?                             (
        <PackagingList
          onClick={() => userAction(item, "report")}
          style={{ cursor: 'pointer' }}
        />
      ): (
        <PackagingList style={{ color: 'grey' }} />
      ) }
            {/* {item.status === "Repack" ?                             (
        <PackagingList
          onClick={() => userAction(item, "report")}
      ...
      <PackerApproval open={visibleReq} onClose={()=>userAction(selectItem, "visible")} item = {selectItem}/>
      <PackerReportingRequest open={reportReq} onClose={()=>userAction(selectItem,"report")} item = {selectItem}/>
